...
class StockCountService extends BaseService
{
    ...
    public function overview(): array
    {
        ...
        $totalLastMonth = StockCount::where(
            'created_at',
            '<',
            now()->startOfMonth()
        )->count();

        $completed = StockCount::where(
            'status',
            'completed'
        )->count();

        $completedLastMonth = StockCount::where(
            'status',
            'completed'
        )
            ->where(
        ...
        $statusCounts = StockCount::select(
            'status',
        ...
        $breakdown = $statusCounts
            ->map(function ($count, $status) use ($total, $statusColors) {
                return [
                    'label' => ucwords(
        ...
        return [
            ...
            'pending_review' => StockCount::where(
                'status',
                'pending_review'
            )->count(),

            'variances_found' => StockCount::whereHas(
                'items',
                fn ($q) => $q->where(
                    'variance_quantity',
                    '!=',
    ...
    public function addItems(
        ...
        if (! $stockCount->isEditable()) {
            throw new \RuntimeException(
                'Items can only be added while the stock count is a draft.'
            );
        }
    ...
    public function removeItem(
        ...
        if (! $stockCount->isEditable()) {
            throw new \RuntimeException(
                'Items can only be removed while the stock count is a draft.'
            );
        }
    ...
    public function updateItemCount(
        ...
        if ($stockCount->status !== 'in_progress') {
            throw new \RuntimeException(
                'Counted quantities can only be recorded while the stock count is in progress.'
            );
        }
    ...
    public function countingProgress(
        StockCount $stockCount
    ): array {
    ...
    public function submit(
        ...
        if ($stockCount->status !== 'in_progress') {
            throw new \RuntimeException(
                'Only an in-progress stock count can be submitted for review.'
            );
        }
        ...
        if (
            ...
            throw new \RuntimeException(
                'Add at least one item before submitting this stock count.'
            );
        }
    ...
    public function approveAndComplete(
        StockCount $stockCount,
        NotificationService $notifications
    ): StockCount {
        if (
    ...
    public function reject(
        ...
        if (
            ...
            throw new \RuntimeException(
                'Only a stock count that is pending review can be rejected.'
            );
        }
    ...
    public function requestRecount(
        ...
        if (
            ...
            throw new \RuntimeException(
                'Only a stock count that is pending review can have a recount requested.'
            );
        }
    ...
    public function populateSystemQuantities(
        StockCount $stockCount
    ): void {
        foreach (
            $stockCount->items
